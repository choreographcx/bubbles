...
function forminatorSignInit() {
	...
	jQuery( '.forminator-button.forminator-button-next, .forminator-step' ).off( 'click.forminatorSign' ).on( 'click.forminatorSign', debounce( function() {
...
function forminatorSignatureHasUserInput( id ) {
	var signatureObject = window[ 'obj' + id ];

	return (
		'undefined' !== typeof signatureObject &&
		'function' === typeof signatureObject.IsValid &&
		signatureObject.IsValid()
	);
}

function forminatorSignatureToggleFilledState( field, id ) {
...
function forminatorSignatureResize() {
	...
	jQuery ( ".forminator-signature" ).each( function() {
		...
		if ( typeof window[ 'obj' + id ] !== "undefined" ) {
			...
			var dataKey = 'forminator-sig-initialized';
			if ( $el.data( dataKey ) ) {
				return;
			}
			...
			$el.off( ".forminatorSignatureState" ).on( "mouseover.forminatorSignatureState", function() {
				// Set hover class.
				jQuery( this ).closest( ".forminator-field" ).addClass( "forminator-is_hover" );
			}).on( "mouseleave.forminatorSignatureState", function() {
				var field = jQuery( this ).closest( ".forminator-field" );

				// Remove hover class.
				field.removeClass( "forminator-is_hover" );
				forminatorSignatureToggleFilledState( field, id );
			});

			$el.find( "#" + id ).off( ".forminatorSignatureState" ).on( "mousedown.forminatorSignatureState mouseup.forminatorSignatureState touchend.forminatorSignatureState", function() {
				...
				setTimeout( function() {
					...
					forminatorSignatureToggleFilledState( sigCanvas.closest( ".forminator-field" ), id );
				}, 50 );
			});
